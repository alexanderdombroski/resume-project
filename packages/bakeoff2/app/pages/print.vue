<template>
  <section class="print-page">
    ...
    <div v-if="resume" class="resume-scroll">
...
const totalMeasuredHeightInches = computed(() => {
  ...
  if (!currentResume || !pretextModule.value || !width) {
    return 0;
  }
  ...
  totalPx += measureText(currentResume.title, width, titleFont, TITLE_LINE_HEIGHT).heightPx;

  if (currentResume.summary.trim()) {
    totalPx += SUMMARY_MARGIN_TOP_PX;
    totalPx += measureText(currentResume.summary, width, bodyFont, BODY_LINE_HEIGHT).heightPx;
  }

  currentResume.sections.forEach((section, sectionIndex) => {
    ...
    if (sectionIndex < currentResume.sections.length - 1) {
